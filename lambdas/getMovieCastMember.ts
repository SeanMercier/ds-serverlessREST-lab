...
export const handler: APIGatewayProxyHandlerV2 = async (event, context) => {
  try {
    ...
    const queryParams = event.queryStringParameters as MovieCastMemberQueryParams;
    ...
    if (!queryParams.movieId) {
      return {
        ...
        body: JSON.stringify({ message: "movieId query parameter is required" }),
      };
    }

    const movieId = parseInt(queryParams.movieId, 10) || 0;
    if (isNaN(movieId) || movieId <= 0) {
      return {
        ...
        body: JSON.stringify({ message: "movieId must be a valid positive number" }),
      };
    }
